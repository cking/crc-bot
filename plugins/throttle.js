...
async function checkMessage(msg) {
  ...
  var channelBuckets = buckets[msg.channel.name]
    ? buckets[msg.channel.name]
    : (buckets[msg.channel.name] = {})

  // do we need to initialise the user bucket?
  var bucket = channelBuckets[msg.author.id]
    ? channelBuckets[msg.author.id]
    : (channelBuckets[msg.author.id] = new TokenBucket(
        config.maxTokens,
        config.tokenInterval,
        msg.author.id
      ))

  // drain the bucket
  if (msg.author.bot) return
  if (!(await bucket.drain())) {
    // failed to drain the bucket - user has exceeded message limit
    msg
      .delete(expire) // dump the response
      .then(msg => console.log(`Deleted message from ${msg.author}`))
    //  Send message to user to notify him
    msg.author.send(
      'Hello ' +
        msg.author +
        ', this is an automated message. Your latest message on CRC has been deleted due to our 24-hour repost rule. You will be able to post again in the LFG/LFM channel in ' +
        Math.ceil((bucket.nextFill - Date.now()) / 1000 / 60) +
        ' minutes. Please check the rules in #announcements for more information.'
    )
  }
}
